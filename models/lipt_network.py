...
from models.lipt_block import LIPTB, MeanShift

def create_model(args):
    return LIPT(args)

class LIPT(nn.Module):
    def __init__(self,args):
        super(LIPT, self).__init__()
            ...
            if (i+1) % 2 == 1: 
                m_body.append(
                    LIPTB(
                        ...
                        self.window_sizes, shared_depth=self.n_share,deploy=self.deploy
                    )
                )
            else:              
                m_body.append(

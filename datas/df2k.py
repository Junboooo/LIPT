...
class DF2K(data.Dataset):
    def __init__(
        self, DIV_HR_folder, DIV_LR_folder,Fli_HR_folder, Fli_LR_folder, CACHE_folder, 
        train=True, augment=True, scale=2, colors=1, 
        patch_size=96, repeat=168
    ):
        super(DF2K, self).__init__()
        self.DIV_HR_folder = DIV_HR_folder
        self.DIV_LR_folder = DIV_LR_folder
        self.Fli_HR_folder = Fli_HR_folder
        self.Fli_LR_folder = Fli_LR_folder
        ...
        if self.train:
            ...
            self.start_idx_D = 801
            self.end_idx_D = 901
            self.start_idx_F = 1
            self.end_idx_F = 1

        for i in range(self.start_idx_D, self.end_idx_D):
            idx = str(i).zfill(4)
            hr_filename = os.path.join(self.DIV_HR_folder, idx + self.img_postfix)
            lr_filename = os.path.join(self.DIV_LR_folder, 'X{}'.format(self.scale), idx + 'x{}'.format(self.scale) + self.img_postfix)
            self.hr_filenames.append(hr_filename)
            self.lr_filenames.append(lr_filename)
        for i in range(self.start_idx_F, self.end_idx_F):
            idx = str(i).zfill(6)
            hr_filename = os.path.join(self.Fli_HR_folder, idx + self.img_postfix)
            lr_filename = os.path.join(self.Fli_LR_folder, 'X{}'.format(self.scale), idx + 'x{}'.format(self.scale) + self.img_postfix)
            self.hr_filenames.append(hr_filename)
            self.lr_filenames.append(lr_filename)
        self.nums_trainset = len(self.hr_filenames)

        LEN = (self.end_idx_D+self.end_idx_F) - (self.start_idx_D+self.start_idx_F)
...
if __name__ == '__main__':
    DIV_HR_folder = '/home/ma-user/work/qiaojunbo/ELAN-main/dataset/DF2K_HR/DIV2K'
    DIV_LR_folder = '/home/ma-user/work/qiaojunbo/ELAN-main/dataset/DF2K_LR_bicubic/DIV2K'
    Fli_HR_folder = '/home/ma-user/work/qiaojunbo/ELAN-main/dataset/DF2K_HR/Flickr2K'
    Fli_LR_folder = '/home/ma-user/work/qiaojunbo/ELAN-main/dataset/DF2K_LR_bicubic/Flickr2K'
    argment   = True
    df2k = DF2K(DIV_HR_folder, DIV_LR_folder, Fli_HR_folder, Fli_LR_folder, augment=True, scale=2, colors=3, patch_size=96, repeat=168, store_in_ram=True)

    print("numner of sample: {}".format(len(df2k)))
    start = time.time()
    for idx in range(10):
        tlr, thr, vlr, vhr = df2k[idx]
